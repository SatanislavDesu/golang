...
// func main() {
// 	x := []string{"a", "b", "c", "d", "e"}
// 	y := x[:1]  // срез от начала до второго элемента
// 	z := x[1:1] // срез от второго до второго элемента (пустой срез)
// 	d := x[1:3] // срез от второго до третьего элемента
// 	e := x[:]   // срез от начала до конца
...
// func main() {
// 	x := []string{"a", "b", "c", "d", "e"}
// 	y := x[:2] // срез от начала до третьего элемента(не включая)
// 	z := x[1:] // срез от второго элемента до конца

// 	x[1] = "y" // изменяем второй элемент
// 	y[0] = "x" // изменяем первый элемент среза y
// 	z[3] = "z" // изменяем четвертый элемент среза z

// 	fmt.Println("x:", x) // измененный срез x
// 	fmt.Println("y:", y) // измененный срез y
// 	fmt.Println("z:", z) // измененный срез z
// 	// все срезы ссылаются на один и тот же массив
// 	// поэтому изменения в одном срезе отражаются на всеx
// 	// срезах, которые ссылаются на этот массив
